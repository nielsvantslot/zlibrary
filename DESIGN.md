...
### Magazine Covers (Recipe Magazines)
A two-part cover, not a note card: a colored masthead band (using the same per-skill jewel-tone as that skill's shelf, or walnut for skill-less "general" magazines) carries the title in reversed foil-stamp type, with a circular ink-stamp badge overlapping its lower edge for found/shelved states. Below it, a flat parchment body panel holds the skill label, unlock effect, and note button — no ruled ledger lines (that read as a sticky note in review; dropped).

### VHS Tapes
A drawn cassette, not a book cover: a black plastic shell (landscape, molded gradient) with two circular reel hubs and a hinged-flap groove along the bottom edge, and a paper label sticker across the upper portion — top-edged in the tape's skill jewel-tone — carrying the title, skill, and XP effect in a typed-label voice (mono, not display serif; a home-labeled tape doesn't get foil-stamp type). Status follows the same physical vocabulary as shelf spines rather than reusing the magazine's ink-stamp: missing is a dashed-outline ghost shell with a bleached label, found is lifted off the surface with a brass "FOUND" tag, shelved sits flush with no further decoration.
